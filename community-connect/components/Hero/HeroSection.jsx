...
const HeroSection = forwardRef((_, ref) => {
  ...
  const getContent = (key) => {
    if (loading) return '';
    if (error) throw new Error(`Failed to fetch content from MongoDB: ${error}`);
  ...
  if (loading) {
    return (
      <section className="pt-18 relative overflow-hidden bg-gradient-to-b from-background to-surface">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-20 md:py-32 text-center relative z-10">
          <div className="text-xl text-primary">Loading hero content from MongoDB...</div>
        </div>
      </section>
    );
  }
  ...
  if (error) {
    return (
      <section className="pt-18 relative overflow-hidden bg-gradient-to-b from-background to-surface">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-20 md:py-32 text-center relative z-10">
          <div className="text-xl text-red-600 bg-red-100 p-6 rounded-lg">
            <h2 className="font-bold mb-2">MongoDB Connection Error</h2>
            <p>Failed to fetch hero content from MongoDB: {error}</p>
  ...
  try {
    ...
    return (
      <section className="pt-18 relative overflow-hidden bg-gradient-to-b from-background to-surface">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-20 md:py-32 text-center relative z-10">
          <div className="text-xl text-red-600 bg-red-100 p-6 rounded-lg">
            <h2 className="font-bold mb-2">Content Missing Error</h2>
            <p>{contentError.message}</p>
            <p className="text-sm mt-2">Hero section requires content to be present in MongoDB. Please add the missing content via the admin panel.</p>
          </div>
        </div>
      </section>
    );
