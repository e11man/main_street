...
const StatItem = ({ target, label, icon, className = '' }) => {
  ...
  return (
    <div className={`text-center p-6 md:p-8 bg-white rounded-xl border border-border-light shadow-sm
                    transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] relative z-10 overflow-hidden
                    hover:translate-y-[-8px] hover:scale-[1.03] hover:shadow-lg hover:border-accent1/30
      ...
      {/* Decorative elements */}
      <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-primary/3 to-accent1/3 rounded-full -translate-x-1/2 -translate-y-1/2 opacity-30"></div>
      <div className="absolute bottom-0 left-0 w-16 h-16 bg-gradient-to-tr from-accent2/3 to-accent1/3 rounded-full translate-x-1/2 translate-y-1/2 opacity-20"></div>
      
      {/* Icon */}
      {icon && (
        <div className="flex justify-center mb-4">
          <div className="p-3 bg-primary/5 rounded-full inline-block">
